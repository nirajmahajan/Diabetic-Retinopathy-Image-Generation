import numpy as np
import torch
import random
import torchvision
import matplotlib.pyplot as plt
from torchvision import transforms, models, datasets
from torch import nn, optim
from torch.nn import functional as F
import pickle
import argparse
import sys
import os
import PIL
import time
import progressbar
from sklearn.cluster import KMeans
from sklearn.mixture import GaussianMixture
from sklearn.decomposition import PCA
import gc

from myModels import VAE
from myDatasets import MESSIDOR_256

def seed_torch(seed=0):
    random.seed(seed)
    os.environ['PYTHONHASHSEED'] = str(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    torch.cuda.manual_seed(seed)
    torch.cuda.manual_seed_all(seed) # if you are using multi-GPU.
    torch.backends.cudnn.benchmark = False
    torch.backends.cudnn.deterministic = True


parser = argparse.ArgumentParser()
parser.add_argument('--resume', action = 'store_true')
parser.add_argument('--resume_from_drive', action = 'store_true')
parser.add_argument('--eval', action = 'store_true')
parser.add_argument('--seed', type = int, default = 0)
parser.add_argument('--state', type = int, default = -1)
parser.add_argument('--id', type = str, default = '1.1')
args = parser.parse_args()

seed_torch(args.seed)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
TRAIN_BATCH_SIZE = 40
lr = 5e-4
SAVE_INTERVAL = 25
NUM_EPOCHS = 2000
MOMENTUM = 0.9
WEIGHT_DECAY = 5e-4
# EXPERIMENT_ID = os.getcwd().split('/')[-1][4:].strip()
EXPERIMENT_ID = args.id
expected_id = '3.1'
assert(expected_id == EXPERIMENT_ID)
losses = []
torch.autograd.set_detect_anomaly(True)


runtime_path = '/content/drive/Shareddrives/RND_2021_niraj_shalabh/tasks/{}/'.format(EXPERIMENT_ID)
if not os.path.isdir(runtime_path):
    os.mkdir(runtime_path)
local_path = './models/'
if not os.path.isdir(local_path):
    os.mkdir(local_path)
if not os.path.isdir('./images/'):
    os.mkdir('./images/')
    os.mkdir('./images/train')
    os.mkdir('./images/test')
dataset_path = '/content/drive/Shared drives/Datasets/'

trainset = MESSIDOR_256(train = None, get_segmentations = True, get_unlabelled = True, train_ratio = 0.75)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=TRAIN_BATCH_SIZE, num_workers = 2, shuffle = True)

model = VAE().to(device)

optimizer = optim.Adam(model.parameters(), lr=lr)
scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', patience = 10, verbose = True, factor=0.5)

bceloss = nn.BCELoss(reduction='sum')
def criterion(reconstruction, images, mean, log_var):
    loss_bce = bceloss(reconstruction.view(-1,256,256), images.view(-1,256,256))
    kld_loss = -0.5 * torch.sum(1 + log_var - mean.pow(2) - log_var.exp())
    return (loss_bce + kld_loss)/images.shape[0]

with open('force_save.txt', 'w') as f:
    f.write('0')

global pre_e
pre_e = 0

if args.resume or args.resume_from_drive:
    if args.resume_from_drive:
        print('Copying the checkpoint to the runtime')
        os.system("cp -r '{}'state.pth* ./models".format(runtime_path))
        model_state = torch.load(local_path + 'state.pth')['state']
        os.system("cp -r '{}'checkpoint_{}.pth ./models".format(runtime_path, model_state))
    if (not args.state == -1):
        model_state = args.state
    print('Loading checkpoint at model state {}'.format(model_state))
    dic = torch.load(local_path + 'checkpoint_{}.pth'.format(model_state))
    pre_e = dic['e']
    model.load_state_dict(dic['model'])
    optimizer.load_state_dict(dic['optimizer'])
    losses = dic['losses']
    print('Resuming Training at {} from epoch {}'.format(pre_e,EXPERIMENT_ID))
else:
    model_state = 0
    pre_e =0
    print('Starting Training at {}'.format(EXPERIMENT_ID))

def is_eval_mode():
    return args.eval

def train(e):
    print('\nTraining for epoch {}'.format(e))
    tot_loss = 0

    bar = progressbar.ProgressBar(maxval=len(trainloader), \
    widgets=[progressbar.Bar('=', '[', ']'), ' ', progressbar.Percentage()])
    bar.start()
    bari = 0

    num = 1
    im = model.generate_new(num)
    
    if e % 10 == 0 or e < 10:
        for i in range(num):
            plt.imshow(im[i], cmap = 'gray')
            plt.savefig('./images/train/epoch_{}_{}.png'.format(e,i))

    for batch_num,(_,segments,_) in enumerate(trainloader):
        segments = segments.to(device)
        optimizer.zero_grad()
        mean, log_var, reconstruction = model(segments)

        loss = criterion(reconstruction, segments, mean, log_var)

        loss.backward()
        optimizer.step()
        tot_loss += loss.item()
        
        bari += 1
        bar.update(bari)

    scheduler.step(tot_loss)
    bar.finish()
    print('Total Loss for epoch = {}'.format(tot_loss/batch_num))
    return tot_loss/batch_num


if args.eval:
    num = 10
    im = model.generate_new(num)
    for i in range(num):
        plt.imshow(im[i], cmap = 'gray')
        plt.savefig('./images/test/{}.png'.format(i))
    os._exit(0)

for e in range(NUM_EPOCHS):

    model_state = e//SAVE_INTERVAL
    if pre_e > 0:
        pre_e -= 1
        continue

    if e % SAVE_INTERVAL == 0:
        seed_torch(args.seed)

    loss_crr = train(e)
    losses.append(loss_crr)

    dic = {}
    dic['e'] = e+1
    dic['model'] = model.state_dict()
    dic['optimizer'] = optimizer.state_dict()
    dic['losses'] = losses


    if (e+1) % SAVE_INTERVAL == 0:
        torch.save(dic, local_path + 'checkpoint_{}.pth'.format(model_state))
        torch.save({'state': model_state}, local_path + 'state.pth')
        print('Saving model to {}'.format(runtime_path))
        print('Copying checkpoint to drive')
        os.system("cp -r ./models/checkpoint_{}.pth '{}'".format(model_state, runtime_path))
        os.system("cp -r ./models/state.pth '{}'".format(runtime_path))
        os.system("cp -r images '{}'".format(runtime_path))

    forced = False
    with open('force_save.txt', 'r') as f:
        a = f.read().strip()
        if a == '1':
            print('Force copying checkpoint to drive')
            torch.save(dic, local_path + 'checkpoint_{}.pth'.format(model_state))
            torch.save({'state': model_state}, local_path + 'state.pth')
            print('Saving model to {}'.format(runtime_path))
            os.system("cp -r ./models/checkpoint_{}.pth '{}'".format(model_state, runtime_path))
            os.system("cp -r ./models/state.pth '{}'".format(runtime_path))            
            os.system("cp -r images '{}'".format(runtime_path))            
            forced = True
    if forced:
        with open('force_save.txt', 'w') as f:
            f.write('0')
