...
if args.resume or args.resume_from_drive:
    ...
    model.optimizer_globalD.load_state_dict(dic['optimizer_globalD'])
    glosses = dic['glosses']
    dlosses = dic['dlosses']
...
def validate():
    print('\nTesting')
    with torch.no_grad():
        fake = model.generate(20).squeeze().cpu().numpy()
...
for e in range(NUM_EPOCHS):
    ...
    dic['optimizer_globalD'] = model.optimizer_globalD.state_dict()
    dic['dlosses'] = dlosses
    dic['glosses'] = glosses
