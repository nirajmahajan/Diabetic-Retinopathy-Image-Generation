...
class ADVAE(nn.Module):
    ...
    def train2(self, e, batch_num, segmentation):
        ...
        lG = self.learn_G(true_space, segmentation)
        return lG, lD
    ...
    def learn_D(self, true_space, fake_space):
        ...
        loss_D_decoded_real = self.criterionGAN(pred_decoded_fake, True)

        loss_D_decoded = loss_D_decoded_real + loss_D_decoded_fake
        loss_D_decoded.backward()
        self.optimizer_globalD.step()
        return loss_D.item()

    def learn_G(self, true_space, segmentation):
        self.switch_off_discriminator()
        self.optimizer_G.zero_grad()
        projection = self.decoder(true_space)
        pred_real = self.discriminator(true_space)
        loss_G_GAN = self.criterionGAN(pred_real, True)
        pred_decoded_real = self.global_discriminator(projection)
        loss_G_decoded_GAN = self.criterionGAN(pred_decoded_real, True)
