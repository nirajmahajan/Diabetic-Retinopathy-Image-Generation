...
runtime_path2 = '/content/drive/Shareddrives/RND_2021_niraj_shalabh/tasks/{}/'.format('3.3.2')
...
if not os.path.isdir(os.path.join(runtime_path,'./images/')):
    ...
    os.mkdir(os.path.join(runtime_path,'./images/test3'))
if not os.path.isdir(os.path.join(runtime_path,'./images/test3')):
    os.mkdir(os.path.join(runtime_path,'./images/test3'))
...
if args.resume_from_drive:
    ...
    os.system("cp -r '{}'state.pth* ./models3".format(runtime_path2))
    model_state2 = torch.load(local_path2 + 'state.pth')['state']
    os.system("cp -r '{}'checkpoint_{}.pth ./models3".format(runtime_path2, model_state2))
model_state = torch.load(local_path + 'state.pth')['state']
model_state2 = torch.load(local_path2 + 'state.pth')['state']
if (not args.state == -1):
...
def validate():
    ...
    with torch.no_grad():
        ...
        for i in range(fake.shape[0]):
            ...
            plt.subplot(1,2,2)
            plt.imshow(f)
            plt.savefig('images/test3/test_{}.png'.format(i))

    os.system("cp -r ./images/train/* '{}'".format(os.path.join(runtime_path,'./images/train/')))
    os.system("cp -r ./images/test3/* '{}'".format(os.path.join(runtime_path,'./images/test3/')))
