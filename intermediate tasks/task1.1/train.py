...
MOMENTUM = 0.9
WEIGHT_DECAY = 5e-4
# EXPERIMENT_ID = os.getcwd().split('/')[-1][4:].strip()
EXPERIMENT_ID = args.id
losses = []
torch.autograd.set_detect_anomaly(True)


runtime_path = '/content/drive/Shared drives/RND_2021_niraj_shalabh/tasks/{}/'.format(EXPERIMENT_ID)
...
if not os.path.isdir('./images/'):
    os.mkdir('./images/')
dataset_path = '/content/drive/Shared drives/Datasets/'
...
testset = datasets.MNIST(dataset_path, download=True, train=False, transform=test_transform)
testloader = torch.utils.data.DataLoader(testset, batch_size=TRAIN_BATCH_SIZE, num_workers = 2)
trainset = datasets.MNIST(dataset_path, download=True, train=True, transform=train_transform)
...
def criterion(reconstruction, images, mean, log_var):
    loss_bce = bceloss(reconstruction.view(-1,28,28), images.view(-1,28,28))
    kld_loss = -0.5 * torch.sum(1 + log_var - mean.pow(2) - log_var.exp())
    return (loss_bce + kld_loss)/images.shape[0]
...
def train(e):
    ...
    for batch_num,(images,_) in enumerate(trainloader):
        images = images.to(device)
        optimizer.zero_grad()
        mean, log_var, reconstruction = model(images)

        loss = criterion(reconstruction, images, mean, log_var)
