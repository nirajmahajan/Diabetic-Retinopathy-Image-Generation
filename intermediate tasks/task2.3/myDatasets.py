...
class EYEPACS_train(Dataset):
    ...
    def __init__(self, train = None, get_segmentations = False, get_unlabelled = False, train_ratio = 0.75, equi_sampling = False, transform = None, drop_grades = None):
        ...
        self.data_path = '/content/drive/Shareddrives/RND_2021_niraj_shalabh/datasets/EYEPACS/'
        self.image_names = []
        self.image_grade = []
        self.get_unlabelled = get_unlabelled
        self.get_segmentations = get_segmentations
        assert(not get_segmentations)
        self.train = train
        self.train_ratio = train_ratio
        self.equi_sampling = equi_sampling
        self.grade_dic = {}
        with open(os.path.join(self.data_path, 'trainLabels.csv'), 'r') as f:
          for i,l in enumerate(f):
            if i == 0:
              continue
            self.grade_dic[l.strip().split(',')[0]] = int(l.strip().split(',')[1])

        for root, dirs, files in os.walk(os.path.join(self.data_path, 'train'), topdown=True):
            for name in sorted(files):
                path = os.path.join(root, name)
                code = self.grade_dic[name.split('.')[0]]
                if code == 'unlabelled':
                    if self.get_unlabelled:
                        code = -1
                    else:
    ...
    def __getitem__(self, i):
        ...
        im = PIL.Image.open(os.path.join(os.path.join(self.data_path, 'train'), self.image_names[index]))
        if self.get_segmentations:
            ims = PIL.Image.open(os.path.join(os.path.join(self.data_path, 'segments/{}'.format(code)), self.image_names[index]))
            return self.transform(im), self.base_transform(ims), self.image_grade[index]
        else:
...
class EYEPACS_train_64(Dataset):
    """docstring for EYEPACS_train_64"""
    def __init__(self, train = None, get_segmentations = False, get_unlabelled = False, train_ratio = 0.75, equi_sampling = False, transform = None, drop_grades = None):
        super(EYEPACS_train_64, self).__init__()
        self.transform = transform
        self.base_transform = transforms.Compose([
                                        transforms.Resize(256),
                                        transforms.ToTensor(),
        ...
        for root, dirs, files in os.walk(os.path.join(self.data_path, 'train64'), topdown=True):
            for name in sorted(files):
                path = os.path.join(root, name)
                code = self.grade_dic[name.split('.')[0]]
                if code == 'unlabelled':
        ...
        for i in range(len(self.image_names)):
            self.image_data.append(PIL.Image.open(os.path.join(os.path.join(self.data_path, 'train64/'), self.image_names[i])))
