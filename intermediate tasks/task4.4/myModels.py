...
class VAE_GAN(nn.Module):
    def __init__(self, lr = 2e-4, latent_space = 32):
        ...
        self.criterionGAN2 = GANloss('vanilla').to(device)
        self.criterionGAN3 = GANloss('vanilla').to(device)
        ...
        self.optimizer_code_D = torch.optim.Adam(self.discriminator_code.parameters(), lr=lr, betas=(.5, 0.999))
        self.optimizer_D = torch.optim.Adam(self.discriminator.parameters(), lr=lr, betas=(.5, 0.999))
    ...
    def generate(self, n):
        ...
        dec = self.decoder(noise).to(device)
        return dec,self.generator(dec)
    ...
    def train(self, e, batch_num, segmentation, retinopathy):
        ...
        lcode = self.learn_code(true_space,fake_space)
    ...
    def learn_D(self, true_space, fake_space, segmentation, retinopathy):
        ...
        loss_D = (loss_D_real2 + loss_D_fake2)/2
        loss_D.backward()
        self.optimizer_D.step()
        return loss_D.item()

    def learn_code(self,true_space,fake_space):
        self.optimizer_code_D.zero_grad()
        pred_fake = self.discriminator_code(fake_space.detach())
        loss_D_fake = self.criterionGAN3(pred_fake, False)

        pred_real = self.discriminator_code(true_space)
        loss_D_real = self.criterionGAN3(pred_real, True)

        loss_D = (loss_D_fake + loss_D_real)/2
        loss_D.backward()
        self.optimizer_code_D.step()
        return loss_D.item()
