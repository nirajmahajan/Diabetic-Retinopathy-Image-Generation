...
if args.resume or args.resume_from_drive:
    ...
    model.optimizer_code_D.load_state_dict(dic['optimizer_code_D'])
    glosses = dic['glosses']
    dlosses = dic['dlosses']
    closses = dic['closses']
...
def train(e):
    ...
    for batch_num,(images,segments,grades) in enumerate(trainloader):
        lg, ld, lc = model.train(e, batch_num, segments.to(device), images.to(device))
        tot_loss_d += ld
        tot_loss_g += lg
        tot_loss_c += lc
        bari += 1
    ...
    print('Total Discriminator Loss for epoch = {}'.format(tot_loss_d+tot_loss_c/batch_num))
    return tot_loss_g/batch_num, tot_loss_d/batch_num, tot_loss_c/batch_num
...
def validate():
    ...
    with torch.no_grad():
        dec,fake = model.generate(20)
        dec = dec.cpu().numpy()
        fake = fake.cpu()
        for i in range(fake.shape[0]):
            f = inv_transform(fake[i])
            d = dec[i]
...
for e in range(NUM_EPOCHS):
    ...
    dic['optimizer_code_D'] = model.optimizer_code_D.state_dict()
    dic['dlosses'] = dlosses
    dic['closses'] = closses
    dic['glosses'] = glosses
